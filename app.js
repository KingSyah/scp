...
function renderPalette(hexColors, container) {
  ...
  hexColors.forEach((hex, idx) => {
    ...
    const doCopy = () => {
      const text = formatColor(hex, copyFormat);
...
function generateSeasonalPalette() {
  ...
  // ── Surprise Strategies: truly random, NOT tied to seasonal palettes ──
  const strategies = [
    // 1. Full chaos: completely random hues, high saturation
    () => {
      return Array.from({length:5}, () => {
        const H = Math.random() * 360;
        const S = 0.55 + Math.random() * 0.40; // 55-95%
        const L = 0.25 + Math.random() * 0.50; // 25-75%
        return rgbToHex(...hslToRgb(H, S, L));
      });
    },
    // 2. Split complementary wild: base hue + 2 complements far apart
    () => {
      const baseH = Math.random() * 360;
      const hues = [
        baseH,
        (baseH + 150 + Math.random()*30) % 360,
        (baseH + 210 + Math.random()*30) % 360,
        (baseH + 90 + Math.random()*40) % 360,
        (baseH + 270 + Math.random()*40) % 360,
      ];
      return hues.map(h => {
        const S = 0.50 + Math.random() * 0.45;
        const L = 0.20 + Math.random() * 0.55;
        return rgbToHex(...hslToRgb(h % 360, S, L));
      });
    },
    // 3. Neon explosion: high saturation, medium-high lightness
    () => {
      return Array.from({length:5}, () => {
        const H = Math.random() * 360;
        const S = 0.80 + Math.random() * 0.20; // 80-100%
        const L = 0.45 + Math.random() * 0.25; // 45-70%
        return rgbToHex(...hslToRgb(H, S, L));
      });
    },
    // 4. Pastel dream: low saturation, high lightness, random hues
    () => {
      return Array.from({length:5}, () => {
        const H = Math.random() * 360;
        const S = 0.25 + Math.random() * 0.35; // 25-60%
        const L = 0.70 + Math.random() * 0.22; // 70-92%
        return rgbToHex(...hslToRgb(H, S, L));
      });
    },
    // 5. Dark moody: low lightness, varied saturation
    () => {
      return Array.from({length:5}, () => {
        const H = Math.random() * 360;
        const S = 0.30 + Math.random() * 0.55; // 30-85%
        const L = 0.10 + Math.random() * 0.30; // 10-40%
        return rgbToHex(...hslToRgb(H, S, L));
      });
    },
    // 6. Analogous rainbow walk: hues spread across full spectrum
    () => {
      const startH = Math.random() * 360;
      return Array.from({length:5}, (_, i) => {
        const H = (startH + i * 72 + (Math.random()*20-10)) % 360; // ~72° apart ±10°
        const S = 0.60 + Math.random() * 0.35;
        const L = 0.30 + Math.random() * 0.45;
        return rgbToHex(...hslToRgb(H, S, L));
      });
    },
  ];

  // Pick a random strategy — pure chaos
  const raw = strategies[Math.floor(Math.random() * strategies.length)]();
  surpriseBaseColors = getEffectiveColors('surprisePalette', raw);

  seasonBadge.textContent = `${data.emoji} ${data.name}`;
  surpriseDesc.textContent = `🎲 Random Surprise! ${data.name} vibes included.`;

  // NOTE: deliberately skip applyThemeToArray for surprise — user wants GENUINE surprise
  // If user wants themed surprise, they can re-apply theme after
  checkDistance(surpriseBaseColors, surpriseDistWarn);
  btnSurpriseExport.style.display = 'flex';
  updateEveSourceButtons();
  return surpriseBaseColors;
}
...
/* ═══════════════════════════════════════════════════════════
   § 14. EVE ONLINE THEME (dedicated section)
   ═══════════════════════════════════════════════════════════ */

const eve1 = document.getElementById('eve1');
const eve2 = document.getElementById('eve2');
const eve3 = document.getElementById('eve3');
const eve4 = document.getElementById('eve4');
const eveChips = document.getElementById('eveChips');
const eveString = document.getElementById('eveString');
const eveCopyBtn = document.getElementById('eveCopyBtn');
const eveFromImg = document.getElementById('eveFromImg');
const eveFromHarmony = document.getElementById('eveFromHarmony');
const eveFromSurprise = document.getElementById('eveFromSurprise');
const eveRandom = document.getElementById('eveRandom');

function getEveColors() {
  return [eve1.value, eve2.value, eve3.value, eve4.value];
}

function updateEvePreview() {
  const colors = getEveColors();
  // Update chips
  eveChips.innerHTML = '';
  colors.forEach(col => {
    const d = document.createElement('div');
    d.className = 'eve-chip';
    d.style.background = col;
    eveChips.appendChild(d);
  });
  // Update string
  eveString.textContent = colors.map(h => h.toUpperCase()).join(',');
}

function setEveColors(hexArray) {
  const c = hexArray.slice(0, 4);
  while (c.length < 4) c.push('#808080');
  eve1.value = c[0];
  eve2.value = c[1];
  eve3.value = c[2];
  eve4.value = c[3];
...
// Source: from image palette
eveFromImg.addEventListener('click', () => {
  if (imgExtractedColors) setEveColors(imgExtractedColors);
});

// Source: from harmony
eveFromHarmony.addEventListener('click', () => {
  if (harmonyBaseColors) setEveColors(harmonyBaseColors);
});

// Source: from surprise
eveFromSurprise.addEventListener('click', () => {
  if (surpriseBaseColors) setEveColors(surpriseBaseColors);
});

// Source: random — 4 completely random colors
eveRandom.addEventListener('click', () => {
  const colors = Array.from({length: 4}, () => {
    const H = Math.random() * 360;
    const S = 0.5 + Math.random() * 0.45;
    const L = 0.25 + Math.random() * 0.50;
    return rgbToHex(...hslToRgb(H, S, L));
  });
  setEveColors(colors);
});
...
[eve1, eve2, eve3, eve4].forEach(picker => {
  picker.addEventListener('input', updateEvePreview);
});

// Copy button
eveCopyBtn.addEventListener('click', () => {
  const str = getEveColors().map(h => h.toUpperCase()).join(',');
  navigator.clipboard.writeText(str).then(() => {
    eveCopyBtn.textContent = '✅ Copied!';
    setTimeout(() => { eveCopyBtn.textContent = '📋 Copy'; }, 1500);
  });
});

// Enable source buttons when palettes are available
function updateEveSourceButtons() {
  eveFromImg.disabled = !imgExtractedColors;
  eveFromHarmony.disabled = !harmonyBaseColors;
  eveFromSurprise.disabled = !surpriseBaseColors;
}
